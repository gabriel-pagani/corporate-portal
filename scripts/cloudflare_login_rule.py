...
O motivo de a regra viver lá e não aqui: o limit_req do nginx conta por IP e o
teto do axes conta por usuário e IP. Quem tem uma lista de endereços recomeça
todos eles em cada endereço novo. O desafio na borda não conta nada: cada
tentativa precisa resolvê-lo antes de chegar aqui, e resolver custa mais do que
trocar de IP. O teto por usuário do app.utils.throttle é o que segura o resto.
...
formulário passa com ele. As telas cobertas são a do login do portal e a do
painel de administração, as duas únicas portas que conferem senha.

Todo acesso ao sistema chega pela Cloudflare, então a regra vale para todo
mundo: não existe caminho que entre no nginx sem passar por ela.
...
DESCRIPTION = 'portal: desafio nas telas de entrada'
